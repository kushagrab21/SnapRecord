...
### R-0027 — `google/gemini-3.8-flash` accepts audio input, and transcribed a recording for USD 0.00044
- **Claim:** OpenRouter's live `/models` list reports `google/gemini-3.8-flash` with `input_modalities` `["text","image","video","file","audio"]`, checked 2026-09-12. Sent a 25,454-byte `audio/mp4` file as an `input_audio` part with `format: "mp4"`, it returned the transcript "This is the ceiling air conditioning unit above the meeting room. Serial plate faces the wall." in 5,538 ms for USD 0.00044325. 46 of the 445 models on the list accept audio; this one was checked first as the packet directed and was used, so no second model was tried.
- **Evidence:** the `/models` query output; `supervision/evidence/P-005-tier2-local.log` line `TRANSCRIBE model=google/gemini-3.8-flash format=mp4 ok=true elapsed_ms=5538 cost_usd=0.00044325`.
- **Caveat:** The audio was NOT a human speaking into a phone. It was macOS `say -v Samantha` rendered to AAC by `afconvert` — clean synthetic speech, no room noise, no clipping, no accent, no distance from the microphone. It says the wire format and the model call work; it says nothing about transcription quality on a real recording made in a real room. Note also that the model's transcript is not a character-for-character echo of the text spoken: `say` was given "...meeting room, serial plate faces the wall" and the model returned two sentences with a full stop. Transcription is a model's reading, and this one already differs from its input in punctuation.

### R-0028 — TIER 1 PROVEN: a dictated note reached a v2-valid row from the phone over plain HTTP
- **Claim:** At 2026-09-12T05:58:47Z an iPhone on the LAN uploaded a photograph with a note over plain HTTP and produced row id=6, `status=valid` against `schema/item.v2.json`, carrying `user_note {text: "Could you tell me what this thing is?", source: "dictation", audio_sha256: null, transcript_model: null}`. Asked whether that text was word-for-word what they said into the microphone, the user answered: "yes, its right".
- **Evidence:** `supervision/evidence/P-005-tier1-phone.log` — `REQ POST /upload remote=192.168.100.21 len=2637211 ct=image/jpeg ua="...GSA/437.4.973319807..."`, then `NOTE source=dictation chars=37 audio=null text="Could you tell me what this thing is?"`, then `ROW id=6 status=valid original_sha256=7f0ba2f7…`. The row is in `supervision/evidence/P-005-rows.json`; the exact prompt sent is `supervision/evidence/P-005-prompt.txt`.
- **Caveat:** Two limits on what this proves. First, the note is a question — "Could you tell me what this thing is?" — not a description, so it carried no disambiguating information and this upload does not show the note improving an extraction; that the note CAN steer the model was seen separately on row id=3, where "this is the ceiling air conditioner in the back room, unit two" produced `item_name: "Ceiling Air Conditioner"`, but that upload came from curl on the laptop, not the phone. Second, the user-agent carries `GSA/437.4`, so this was the Google app's WebView again (R-0021), and dictation in Safari proper on this handset remains untested.

### R-0029 — TIER 2 NOT PROVEN ON THE DEVICE: the phone never reached the HTTPS listener
- **Claim:** Tier 2 works end-to-end on the server and is unproven on the handset. Proven: the HTTPS listener answers on `https://192.168.100.104:8789/`; a framed body (8-byte big-endian image length, image, audio) was parsed; the audio was stored at `data/audio/f4788beb…m4a` and its hash recomputed to `f4788beb967d8252a175949fb1ad64db09b2203ca2b5474338079e3f784d32ac`, matching the row; the transcript became `user_note.text` with `source: "transcription"`; row id=5 validated under v2. Not proven: that Safari will accept the self-signed certificate on this phone, and that `getUserMedia`/`MediaRecorder` are available and permitted there.
- **Evidence:** `supervision/evidence/P-005-tier2-local.log` for the proven half. For the unproven half, the absence: the server log contains no request to port 8789 from `192.168.100.21` or from any address other than the laptop itself. The user was asked twice to open the HTTPS URL in Safari and report what they saw; they did not do so, and instead ended the packet with "yes, its right , now update the readme and push to git".
- **Caveat:** This is NOT the failure mode packet step 4d describes. Nothing was observed to fail — Safari did not refuse the certificate, `getUserMedia` was not found absent, permission was not denied. The device half was simply never attempted, so there is no user statement and no symptom to record, and the honest status is untested rather than failed. The distinction matters: a FAILED result would be evidence that recording cannot work on this handset, and no such evidence exists. The shipped path is Tier 1 either way. Everything in the `MediaRecorder` branch of the page — the button, the touch handlers, the framing done in the browser rather than in Node — has never run in any browser and is claimed only as written code, per R-0026.
